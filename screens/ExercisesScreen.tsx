    ...
    <SafeAreaView className="flex-1 bg-[#0f172A]">
      <ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false}>
        ...
        <ScrollView
          ...
          {categories.map((cat) => (
            <TouchableOpacity
              ...
              <Text
                className={`font-bold ${activeTab === cat.id ? 'text-[#0f172A]' : 'text-slate-400'}`}>
                {cat.title}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <View className="mb-6 rounded-3xl border border-white/10 bg-white/5 p-2">
          <View className="flex-row items-center rounded-2xl border border-[#22C55E30] bg-[#0F172A80] p-4">
